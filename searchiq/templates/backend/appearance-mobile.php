...
if (!empty($_POST) && isset($_POST['btnSubmitMobileStyleOptions']) && check_admin_referer($this->updateMobileNonce)) {
    ...
    $error = array();
    if (
        isset($offsetStyling['searchIconTopOffset']) && $offsetStyling['searchIconTopOffset']
        && isset($offsetStyling['searchIconTopOffsetUnit']) && $offsetStyling['searchIconTopOffsetUnit']
    ) {
        if (
            $offsetStyling['searchIconTopOffset'] !== $this->mobileStyling['searchIconTopOffset']['default']
    ...
    if (isset($_POST['search_icon_selector']) && !empty($_POST['search_icon_selector'])) {
        update_option($this->pluginOptions['search_icon_selector'], sanitize_text_field(wp_unslash($_POST['search_icon_selector'])));
    } else {
...
$styling					= $this->getMobileStyling($stylingVar);
 ...
 ?>
<div class="wsplugin">
 <h2>SearchIQ: Mobile <a class="helpSign userGuide" target="_blank" style="text-decoration: none" href="<?php echo esc_url( $this->userGuideLink );?>"><img style="vertical-align:bottom" src="<?php echo esc_url( SIQ_BASE_URL.'/assets/'.SIQ_PLUGIN_VERSION.'/images/help/help-icon.png'); //phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>" alt="user guide"> User Guide</a></h2>
 <div class="dwAdminHeading">You can change appearance for mobile here.</div>
 <form method="POST" novalidate action="<?php echo esc_url( admin_url( 'admin.php?page=searchiq-mobile-config') ); ?>" onsubmit="return SIQ_validateMobileSettingsForm();" class="custom_page_options" name="custom_options">
	<div class="section section-1">
		<h2>Mobile</h2>
        <div class="data">
            <label>Enable</label>
            <input type="checkbox" name="mobile_enabled" value="yes" <?php echo esc_html( $this->isMobileEnabled() ? "checked=checked": "");?> />
        </div>

        <div class="data">
            <label>Search Bar Placeholder</label>
            <input type="text" value="<?php echo esc_html( $styling['barPlaceholder'] );?>" class="long-input" name="styling[barPlaceholder]"/>
        </div>
        <div class="data">
            <label>Search Result Header Text</label>
            <input type="text" value="<?php echo esc_html( $styling['SearchHeadTxt'] );?>" class="long-input" name="styling[SearchHeadTxt]"/>
        </div>
        <div class="data">
            <label>Search Result Stat Text</label>
            <input type="text" value="<?php echo esc_html( $styling['SearchResultStatTxt'] );?>" class="long-input" name="styling[SearchResultStatTxt]"/>
        </div>
        <div class="data">
            <label>Search Result Loading text</label>
            <input type="text" value="<?php echo esc_html( $styling['SearchLoadingTxt'] );?>" class="long-input" name="styling[SearchLoadingTxt]"/>
        </div>
        <div class="data">
            <label>Search Bar Placeholder Text Color</label>
            <input type="text" value="<?php echo esc_attr( $styling['barPlaceholderTextColor'] );?>" name="styling[barPlaceholderTextColor]" class="color {required:false}"/>
        ...
        <div class="data">
            ...
            <select class="auto-width" id="mobile_favicon_mode" name="mobile_favicon_mode" onchange="var custom = document.getElementById('mobileSearchBarFavicon'); this.value == '2' ? custom.setAttribute('type', 'text') : custom.setAttribute('type', 'hidden');">
                <option value="0" <?php echo esc_attr( $this->pluginSettings['mobile_favicon_mode'] == 0 ? "selected" : "" );?>>Disable</option>
                <option value="1" <?php echo esc_attr( $this->pluginSettings['mobile_favicon_mode'] == 1 ? "selected" : "" );?>>Auto-detect</option>
                <option value="2" <?php echo esc_attr( $this->pluginSettings['mobile_favicon_mode'] == 2 ? "selected" : "" );?>>Custom URL</option>
            </select>
            <input class="long-input" id="mobileSearchBarFavicon" type="<?php echo esc_attr( $this->pluginSettings['mobile_favicon_mode'] == 2 ? "text" : "hidden" );?>" name="styling[barFavicon]" value="<?php echo esc_attr( $styling['barFavicon'] );?>"/>
        </div>
        <div class="data">
			<label>Search Bar Background</label>
			<input type="text" value="<?php echo esc_attr( $styling['barBgColor'] ); ?>" name="styling[barBgColor]" class="color {required:false}"/>
			<div class="clearColor"><span></span></div>
		</div>
        <div class="data">
            <label>Search Input Background</label>
            <input type="text" value="<?php echo esc_attr( $styling['barInputBgColor'] );?>" name="styling[barInputBgColor]" class="color {required:false}"/>
            <div class="clearColor"><span></span></div>
        </div>
        <div class="data">
            <label>Search Input Text Color</label>
            <input type="text" value="<?php echo esc_attr( $styling['barInputTextColor'] );?>" name="styling[barInputTextColor]" class="color {required:false}"/>
            <div class="clearColor"><span></span></div>
        </div>
        <div class="data">
        ...
        <div class="data">
            ...
            <input type="checkbox" name="mobile_float_bar_enabled" value="yes" <?php echo esc_attr( $this->isMobileFloatBarEnabled() ? "checked=checked" : "" );?> />
        </div>

        <div class="data">
            <label>Enable Float Search Icon</label>
            <input type="checkbox" name="mobile_icon_enabled" value="yes" <?php echo esc_attr( $this->isMobileIconEnabled() ? "checked=checked" : "" );?> />
        </div>
		
		<div class="data">
            <label>Search icon selector</label>
            <input type="text" name="search_icon_selector" value="<?php echo esc_attr( $settings['search_icon_selector'] ); ?>" />
        </div>
		
        <div class="data">
            <label>Search icon box background</label>
            <input value="<?php echo esc_attr( $styling['searchIconBoxBg'] ? $styling['searchIconBoxBg'] : $this->mobileStyling['searchIconBoxBg']['default'] ); ?>" name="styling[searchIconBoxBg]" class="color {required:false}"/>
            <div class="clearColor"><span></span></div>
        </div>
        <div class="data">
            <label>Search icon color</label>
            <input value="<?php echo esc_attr( $styling['searchIconColor'] ? $styling['searchIconColor'] : $this->mobileStyling['searchIconColor']['default'] ); ?>" name="styling[searchIconColor]" class="color {required:false}"/>
            <div class="clearColor"><span></span></div>
        </div>
        <div class="data">
            <label>Position offset from top</label>
            <input value="<?php echo esc_attr( $styling['searchIconTopOffset'] );?>" type="number" name="styling[searchIconTopOffset]" onblur="if(!/(^(\d+\.?)(\d+)?$|^(\d+)?(\.\d+)$)/.test(this.value)){this.value=this.value.replace(/[^\d\.]*/, '');}"/>
            <select name="styling[searchIconTopOffsetUnit]" style="max-width: 50px;top: -3px;position: relative;">
                <option <?php echo esc_attr( $styling['searchIconTopOffset'] && $styling['searchIconTopOffsetUnit'] == "%" || !$styling['searchIconTopOffset'] && $this->mobileStyling['searchIconTopOffsetUnit']['default'] == "%" ? "selected" : "" );?> value="%">%</option>
                <option <?php echo esc_attr( $styling['searchIconTopOffset'] && $styling['searchIconTopOffsetUnit'] == "px" || !$styling['searchIconTopOffset'] && $this->mobileStyling['searchIconTopOffsetUnit']['default'] == "px" ? "selected" : "" );?> value="px">px</option>
            </select>
        </div>
	</div>
